...
if (!buildFromSource()) {
	proc.exec('ene-node-gyp-build-test', function (err, stdout, stderr) {
		if (err) {
...
function build() {
	...
	try {
		...
		args = [process.execPath, path.join(require.resolve('node-gyp/package.json'), '..', typeof pkg.bin === 'string' ? pkg.bin : pkg.bin['node-gyp']), 'rebuild'];
	} catch (_) {}
